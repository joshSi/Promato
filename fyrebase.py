#Modified Pyrebase from https://github.com/thisbejim/Pyrebase
import requests
...
class Auth:
    ...
    def create_user_with_email_and_password(self, email, password, name):
        request_ref = "https://www.googleapis.com/identitytoolkit/v3/relyingparty/signupNewUser?key={0}".format(self.api_key)
        headers = {"content-type": "application/json; charset=UTF-8" }
        data = json.dumps({"email": email, "password": password, "displayName": name, "returnSecureToken": True})
        request_object = requests.post(request_ref, headers=headers, data=data)
        raise_detailed_error(request_object)
        return request_object.json()
